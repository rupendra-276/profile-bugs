...
// // components/ProfileTopSection.jsx
// "use client";
...
//   const [cover, setCover] = useState(user?.cover || user?.coverPhoto || "");
...
//   useEffect(() => {
//     setCover(user?.cover || user?.coverPhoto || "");
//     setAvatar(user?.avatar || user?.profilePhoto || "");
//   }, [user]);
  ...
  return (
    <div className="relative">
      ...
      <div className="relative overflow-hidden w-full h-32 md:h-[176px] rounded-[30px] border border-gray-400 bg-[#10151B] group">
        ...
        {/* Cover Button */}
        <div
          className={`absolute ${!cover ? "top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" : ""}`}
          onClick={() => coverInputRef.current.click()}
        >
          <div className="bg-[#10151Baa] p-2 rounded-full cursor-pointer hover:scale-105 transition">
            {!cover && <RiCameraAiLine className="text-white text-xl" />}
          </div>
      ...
      <div className="absolute z-30 -bottom-12 left-8 md:left-14 w-24 h-24 md:w-[112px] md:h-[112px] rounded-[30px] border-2 border-gray-400 bg-[#10151B] group overflow-hidden">
        {avatar ? (
          <img src={avatar} alt="Avatar" className="w-full h-full object-cover rounded-[30px]" />
        ) : (
          <div className="w-full h-full bg-[#10151B] rounded-[30px]" />
        )}
        ...
        {/* Avatar Button */}
        {!avatar && (
          <div className="absolute inset-0 flex justify-center items-center transition" onClick={() => avatarInputRef.current.click()}>
            <div className="bg-[#10151B] p-2 rounded-full cursor-pointer">
              <RiCameraAiLine className="text-white text-xl" />
            </div>
      ...
      {/* Contact Info Modal */}
      <ContactInfoModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} user={user} onSave={handleSaveContactInfo} />
    </div>
  );
}
