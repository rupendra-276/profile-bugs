...
//         No education added yet. to get started.
//       </div>
//     );
//   }
...
//                   {edu.grade && <p className="text-gray-300 text-[12px]">Grade: {edu.grade}</p>}
//                   {edu.achievements && (
//                     <p className="text-gray-300 text-[12px] mt-1">Achievements: {edu.achievements}</p>
//                   )}
//                 </div>
...
//   <div className="mt-2">
//     <a
//       href={typeof edu.media === "string" ? edu.media : URL.createObjectURL(edu.media)}
//       target="_blank"
//       rel="noopener noreferrer"
//       className="inline-block"
//     >
//       <Image
//         src={typeof edu.media === "string" ? edu.media : URL.createObjectURL(edu.media)}
//         alt="Certificate"
//         width={160}
//         height={100}
...
export default function EducationList({ educations, onAdd, onEdit, onDelete, profileUser }) {
  ...
  return (
    <div className="relative">
        ...
        {displayed.map((edu, idx) => (
          <div key={idx} className="relative mb-12 pl-8">
            ...
            <div className="absolute -left-[30px] ms-1 flex items-center justify-center">
              {edu.logo ? (
                <Image src={edu.logo} alt={edu.institution} width={50} height={50} className="rounded-2xl object-cover" />
              ) : (
                <span className="w-[50px] h-[50px] flex items-center justify-center bg-[#31298a] text-white rounded-2xl text-sm font-bold">
                  {edu.institution?.charAt(0)?.toUpperCase() || "E"}
                </span>
            ...
            <div className="text-white">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {edu.grade && <p className="text-gray-300 text-[12px]">Grade: {edu.grade}</p>}
                  {edu.achievements && <p className="text-gray-300 text-[12px] mt-1">Achievements: {edu.achievements}</p>}
                </div>
      ...
      {educations.length > 2 && (
        <div className="text-center mt-4">
          <button onClick={() => setShowAll(!showAll)} className="text-white font-semibold hover:underline">
            {showAll ? "Show less" : `Show all ${educations.length} records`}
          </button>
        </div>
      )}
