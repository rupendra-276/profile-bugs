...
// export default function ExperienceList({ experiences, onAdd, onEdit, onDelete }) {
//   const [showAll, setShowAll] = useState(false);

//   if (!experiences.length) {
//     return (
//       <div className="p-4 text-gray-400 ">
//         No experience added yet.to get started.
//       </div>
//     );
//   }
...
//               <div className="ms-6">
//                 <h4 className="font-semibold text-lg text-white">{exp.jobTitle}</h4>
...
//                       {exp.keyAchievements.slice(0, 3).map((item, i) => (
...
//                   <div className="mt-2">
//                     <a
//       href={typeof exp.media === "string" ? exp.media : URL.createObjectURL(exp.media)}
...
//                     >
//                       <Image
//         src={typeof exp.media === "string" ? exp.media : URL.createObjectURL(exp.media)}
//                         alt="Certificate"
//                         width={160}
//                         height={100}
...
export default function ExperienceList({ experiences, onAdd, onEdit, onDelete, profileUser }) {
  ...
  return (
    <div>
        ...
        {(showAll ? experiences : experiences.slice(0, 2)).map((exp, idx) => (
          <div key={idx} className="relative pl-6">
            ...
            <div className="absolute -left-[30px] ms-1 flex items-center justify-center">
              {exp.logo ? (
                <Image src={exp.logo} alt={exp.company} width={50} height={50} className="rounded-2xl object-cover" />
              ) : (
                <span className="w-[50px] h-[50px] flex items-center justify-center bg-[#077bda] text-white rounded-2xl text-sm font-bold">
                  {exp.company?.charAt(0)?.toUpperCase() || "C"}
                </span>
            ...
            <div className="flex justify-between gap-10 items-start">
              <div className="ms-6">
                ...
                {exp.skills?.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {exp.skills.slice(0, 3).map((skill, i) => (
                      <span key={i} className="px-2 border border-gray-400 pt-1 text-white text-[10px] rounded-full">
                        {skill}
                      </span>
                    ))}
                    {exp.skills.length > 3 && (
              ...
              {currentUser && (
                <div className="flex gap-2">
                  ...
                  <button onClick={() => onDelete(idx)} className="p-1 border rounded-lg border-red-300 text-red-700">
                    <MdDelete />
                  </button>
                </div>
              )}
      ...
      {experiences.length > 2 && (
        <div className="text-center mt-4">
          <button onClick={() => setShowAll(!showAll)} className="text-blue-600 font-semibold hover:underline">
            {showAll ? "Show less" : `Show all ${experiences.length} experiences`}
          </button>
        </div>
      )}
