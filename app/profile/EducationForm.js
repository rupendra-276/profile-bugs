...
// const UNIVERSITIES = ["Harvard University", "Stanford University", "MIT", "IIT", "Oxford University"];
// const LOCATIONS = ["Cambridge, MA", "Bangalore, India", "London, UK", "New York, USA", "Remote"];

// // ----------------- Component -----------------
// const EducationForm = forwardRef(({ initialData = null, onSave, onCancel }, ref) => {
//   const empty = {
//     institution: "",
//     university: "",
//     educationLevel: "",
...
//     achievements: "",
//     media: null, // can be File object or URL
//   };
...
//       const dataset = fieldName === "university" ? UNIVERSITIES : fieldName === "location" ? LOCATIONS : [];
...
//       setErrors((prev) => ({ ...prev, media: `File size must be under ${MAX_FILE_SIZE_MB}MB` }));
//       return;
//     }
...
//     if (!formData.university?.trim()) e.university = "University / Board is required.";
//     if (!formData.educationLevel?.trim()) e.educationLevel = "Education level is required.";
//     if (!formData.fieldOfStudy?.trim()) e.fieldOfStudy = "Field of study is required.";
//     if (!formData.startDate.month || !formData.startDate.year) e.startDate = "Start date is required.";
//     if (!formData.currentlyStudying && (!formData.endDate.month || !formData.endDate.year))
//       e.endDate = "End date required.";
//     if (formData.media && formData.media.size > MAX_FILE_SIZE_MB * 1024 * 1024)
...
//     // ✅ Convert File to URL string if persistence is required
//     const finalData = {
//       ...formData,
...
//         {/* Institution with suggestions */}
//         <div className="relative">
//           <InputWithCount
//             label="Institution Name *"
//             name="institution"
//             value={formData.institution}
//             onChange={(val) => handleChange({ target: { name: "institution", value: val } })}
...
//           />
//           {isFocusedField === "institution" && suggestions.institution.length > 0 && (
...
//             onChange={(val) => handleChange({ target: { name: "university", value: val } })}
...
//             onBlur={() => setTimeout(() => setIsFocusedField(""), 100)}
//           />
//           {isFocusedField === "university" && suggestions.university.length > 0 && (
//             <div className="absolute z-50 left-0 right-0 bg-[#070C11] rounded-md shadow-md mt-1 overflow-hidden">
...
//           onChange={(val) => handleChange({ target: { name: "fieldOfStudy", value: val } })}
...
//           onChange={(val) => handleChange({ target: { name: "specialization", value: val } })}
...
//             onBlur={() => setTimeout(() => setIsFocusedField(""), 100)}
//           />
//           {isFocusedField === "location" && suggestions.location.length > 0 && (
//             <div className="absolute z-50 left-0 right-0 bg-[#070C11] rounded-md shadow-md mt-1 overflow-hidden">
...
//           onChange={(val) => setFormData((p) => ({ ...p, achievements: val }))}
...
const EducationForm = forwardRef(({ initialData = null, onSave, onCancel }, ref) => {
  ...
  const validate = () => {
    ...
    if (formData.currentlyStudying) {
      if (!formData.expectedGraduation.month || !formData.expectedGraduation.year)
      ...
      if (eYear < sYear) e.expectedGraduation = "Expected graduation cannot be before start date.";
    } else {
      if (!formData.endDate.month || !formData.endDate.year) e.endDate = "End date is required.";
      const sYear = parseInt(formData.startDate.year || "0", 10);
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        <div className="relative">
          ...
          {isFocusedField === "institution" && suggestions.institution.length > 0 && (
            <div className="absolute z-50 left-0 right-0 bg-[#070C11] rounded-md shadow-md mt-1 overflow-hidden">
              {suggestions.institution.map((s, i) => (
                <div key={i} className="p-2 text-gray-100 cursor-pointer text-sm" onClick={() => applySuggestion("institution", s)}>
                  {s}
                </div>
              ))}
            </div>
        ...
        <div className="relative">
          ...
          {isFocusedField === "university" && suggestions.university.length > 0 && (
            <div className="absolute z-50 left-0 right-0 bg-[#070C11] rounded-md shadow-md mt-1 overflow-hidden">
              {suggestions.university.map((s, i) => (
                <div key={i} className="p-2 text-gray-100 cursor-pointer text-sm" onClick={() => applySuggestion("university", s)}>
                  {s}
                </div>
              ))}
            </div>
        ...
        <div className="relative">
          ...
          {isFocusedField === "location" && suggestions.location.length > 0 && (
            <div className="absolute z-50 left-0 right-0 bg-[#070C11] rounded-md shadow-md mt-1 overflow-hidden">
              {suggestions.location.map((s, i) => (
                <div key={i} className="p-2 text-gray-100 cursor-pointer text-sm flex items-center gap-2" onClick={() => applySuggestion("location", s)}>
                  <MapPin className="w-4 h-4 text-blue-600" />
                  <span>{s}</span>
                </div>
              ))}
